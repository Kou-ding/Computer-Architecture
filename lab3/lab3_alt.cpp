...
#pragma HLS LOOP_TRIPCOUNT min = 1 max = 64
            ...
            for (int j = 0; j < chunk_size; j++) {
                ...
                // C_local[real_j-1]
                // C_local[real_j+1]
                // C_local[real_j-(WIDTH/VECTOR_SIZE)]
                // C_local[real_j+(WIDTH/VECTOR_SIZE)]
...
#pragma HLS UNROLL
                    ...
                    if(is_interior(ichi)){
                        ...
                        tmpUp = C_local[real_j-(WIDTH/VECTOR_SIZE)].range(32 * (vector + 1) - 1, vector * 32);

                        tmpDown = C_local[real_j+(WIDTH/VECTOR_SIZE)].range(32 * (vector + 1) - 1, vector * 32);

                        // Go to the previous 512bit to find the left element
                        if(vector==0){
                            tmpLeft = C_local[real_j-1].range(511, 480);
                            tmpRight = C_local[real_j].range(32 * ((vector+1)+1) - 1, (vector+1) * 32);
                        // Go to the next 512bit to find the left element
                        }else if(vector==VECTOR_SIZE-1){
                            tmpRight = C_local[real_j+1].range(31, 0);
                            tmpLeft = C_local[real_j].range(32 * ((vector-1)+1) - 1, (vector-1) * 32);
